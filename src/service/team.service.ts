...
export class TeamService {
  constructor(private readonly teamInviteQueryRepository: TeamInviteQueryRepository) {}

  async getTeamInviteInfo(id: number, teamInviteId: number): Promise<GetTeamInviteInfoResponse> {
    const { sendMemberId, sendMemberNickname, postId, postTitle, message } =
      await this.teamInviteQueryRepository.getTeamInviteInfo(id, teamInviteId);

    return new GetTeamInviteInfoResponse(sendMemberId, sendMemberNickname, postId, postTitle, message);
  }
